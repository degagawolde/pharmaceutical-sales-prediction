...
class FeatureEngineering:
    ...
    def generate_columns(self,df:pd.DataFrame) -> None:
        ...
        df.loc[:, ['Year']] = pd.datetime(
            df['Date'], format='%Y-%m-%d').dt.year
        df.loc[:, ['Month']] = pd.datetime(
            df['Date'], format='%Y-%m-%d').dt.month
        df.loc[:, ['WeekOfYear']] = pd.datetime(
            df['Date'], format='%Y-%m-%d').dt.isocalendar().week
        df.loc[:, ['is_month_end']] = pd.datetime(
            df['Date'], format='%Y-%m-%d').dt.is_month_end
        df.loc[:, ['is_month_start']] = pd.datetime(
            df['Date'], format='%Y-%m-%d').dt.is_month_start
        df.loc[:, ['is_quarter_end']] = pd.datetime(
            df['Date'], format='%Y-%m-%d').dt.is_quarter_end
        df.loc[:, ['is_quarter_start']] = pd.datetime(
            df['Date'], format='%Y-%m-%d').dt.is_quarter_start
        df.loc[:, ['is_year_end']] = pd.datetime(
            df['Date'], format='%Y-%m-%d').dt.is_year_end
        df.loc[:, ['is_year_start']] = pd.datetime(
            df['Date'], format='%Y-%m-%d').dt.is_year_start 
