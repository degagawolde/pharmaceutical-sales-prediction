...
class FeatureEngineering:
    ...
    def drop_columns(self,df:pd.DataFrame) -> None:
        df = self.df.drop(self.columns_to_drop,axis=1)
        logger.info(
    ...
    def generate_columns(self,df:pd.DataFrame) -> None:
        ...
        df = self.create_holiday_distance_cols(df)
    ...
    def create_holiday_distance_cols(self,df:pd.DataFrame) -> None:
        ...
        for date in unique_dates:
            after_holiday, to_next_holiday = self._get_holiday_distances(date,holidays)
