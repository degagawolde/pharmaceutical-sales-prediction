...
class FeatureEngineering:
    ...
    def generate_columns(self,df:pd.DataFrame) -> None:
        ...
        df.loc[:, ['Year']] = self.df['Date'].dt.year
        df.loc[:, ['Month']] = self.df['Date'].dt.month
        df.loc[:, ['WeekOfYear']] = self.df['Date'].dt.isocalendar().week
        df.loc[:, ['is_month_end']] = self.df['Date'].dt.is_month_end
        df.loc[:, ['is_month_start']] = self.df['Date'].dt.is_month_start
        df.loc[:, ['is_quarter_end']] = self.df['Date'].dt.is_quarter_end
        df.loc[:, ['is_quarter_start']
                    ] = self.df['Date'].dt.is_quarter_start
        df.loc[:, ['is_year_end']] = self.df['Date'].dt.is_year_end
        df.loc[:, ['is_year_start']] = self.df['Date'].dt.is_year_start
